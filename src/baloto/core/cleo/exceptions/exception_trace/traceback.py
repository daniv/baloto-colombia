...
from rich.console import RenderResult, ConsoleRenderable, group, Group
...
from rich.traceback import Trace, LOCALS_MAX_LENGTH, LOCALS_MAX_STRING, PathHighlighter, Frame, _SyntaxError, \
    _iter_syntax_lines
...
class Traceback:
    ...
    def __rich_console__(
        ...
        for last, stack in loop_last(reversed(self.trace.stacks)):
            if stack.frames:
                # yield self._render_stack(stack),
                stack_renderable: ConsoleRenderable = Panel(
                    self._render_stack(stack),
                    title="[traceback.title]Traceback [dim](most recent call last)",
    ...
    def _render_stack(self, stack: Stack) -> RenderResult:
        ...
        for frame_index, frame in enumerate(stack.frames):
            ...
            if os.path.exists(frame.filename):
                text = f"{frame.filename}:{frame.lineno} in {frame.name}"
                # text = Text.assemble(
                #     frame.filename,
                #     (":", "pygments.text"),
                #     (str(-9999999), "pygments.number"),
                #     " in ",
