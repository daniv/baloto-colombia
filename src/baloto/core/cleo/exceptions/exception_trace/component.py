...
from baloto.core.cleo.exceptions.exception_trace.frame_collection import FrameCollection


if TYPE_CHECKING:
    from baloto.core.cleo.io.io import IO
    from baloto.core.cleo.io.outputs.output import Output
    from baloto.core.cleo.exceptions.exception_trace.frame import Frame
...
class Highlighter:
    ...
    DEFAULT_THEME: ClassVar[dict[str, str]] = {
        TOKEN_STRING: "yellow bold",
        TOKEN_NUMBER: "blue bold",
        TOKEN_COMMENT: "bold italic",
        TOKEN_KEYWORD: "magenta bold",
        TOKEN_BUILTIN: "bold dim",
        TOKEN_DEFAULT: "default",
        TOKEN_OP: "dim bold",
        LINE_MARKER: "red bold",
        LINE_NUMBER: "dim",
    }
...
class ExceptionTrace:
    ...
    def render(self, io: IO | Output, simple: bool = False) -> None:
        ...
        if simple_string := str(self._exception) if simple else "":
            io.write("")
            io.write(f"[error]{simple_string}[/error]")
        else:
            self._render_exception(io, self._exception)

    def _render_exception(self, io: IO | Output, exception: BaseException) -> None:
        from baloto.core.cleo.exceptions.exception_trace.inspector import Inspector
        ...
        # )
        self._render_line(io, f"[b]{exception_message}[/]")
    ...
    def _render_snippet(self, io: IO | Output, frame: Frame) -> None:
        self._render_line(
            io,
            f"at [green]{self._get_relative_file_path(frame.filename)}[/]"
            f":[b]{frame.lineno}[/] in [cyan]{frame.function}[/]",
            True,
        )

        code_lines = Highlighter(supports_utf8=True).code_snippet(
    ...
    def _render_trace(self, io: IO | Output, frames: FrameCollection) -> None:
        ...
        if io.is_very_verbose() and remaining_frames_length:
            self._render_line(io, "[yellow]Stack trace[/]:", True)
            ...
            for collection in frame_collections:
                if collection.is_repeated:
                    if len(collection) > 1:
                        frames_message = f"[yellow]{len(collection)}[/] frames"
                    else:
                        frames_message = "frame"

                    self._render_line(
                        io,
                        f"[blue]{'...':>{max_frame_length}}[/]  "
                        f"Previous {frames_message} repeated "
                        f"[blue]{collection.repetitions + 1}[/] times",
                        True,
                    )
                ...
                for frame in collection:
                    ...
                    relative_file_path = f"[bold dim]{Formatter.escape(os.sep)}[/]".join(
                        (
                            *relative_file_path_parts[:-1],
                            f"[bold]{relative_file_path_parts[-1]}[/]",
                        )
                    )
                    self._render_line(
                        io,
                        f"[yellow]{i:>{max_frame_length}}[/]  "
                        f"{relative_file_path}[bold dim]:[/]"
                        f"[b]{frame.lineno}[/] in [cyan]{frame.function}[/]",
                        True,
                    )
                    ...
                    if io.is_debug():
                        ...
                        highlighter = Highlighter(supports_utf8=True)
                        try:
